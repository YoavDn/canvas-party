...
export function useFireworks(c:CanvasRenderingContext2D,canvas: HTMLCanvasElement) {   
let pointerX = 0;
let pointerY = 0;
...
if(isActive) return 
  animateParticules(
    anime.random(0, canvas.width), 
    anime.random(0, canvas.height))
...
function animateParticules(x : number, y: number) {
  ...
  anime.timeline()
   .add({
    targets: particules,
    x: function(p: pType) { return p.endPos!.x  },
    y: function(p: pType) { return p.endPos!.y; },
    radius: 0.1,
