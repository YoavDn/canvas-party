...
export function useTrippy(c: CanvasRenderingContext2D, canvas: HTMLCanvasElement) {
  const mapSize = 1024;
  ...
  const interpolate = (c1: rgbType, c2: rgbType, f: number) => {
    return {
  ...
  const makeFiveColorGradient = (c1: rgbType, c2: rgbType, c3: rgbType, c4: rgbType, c5: rgbType) => {
    const g = [];
  ...
  const updateImageData = () => {
    for (let u = 0; u < imgSize; u++) {
      for (let v = 0; v < imgSize; v++) {
        ...
        image.data[j] = clr.r;
        image.data[j + 1] = clr.g;
        image.data[j + 2] = clr.b;
      }
    }
  };

  // helper to create a linear gradient palette
  const linearGradient = (c1: rgbType, c2: rgbType) => {
    const g = [];
