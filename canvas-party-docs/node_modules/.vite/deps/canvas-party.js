...
// node_modules/canvas-party/lib/tamplates/confetti.js
var NUM_CONFETTI = 350;
var COLORS = ["#ff579fd9", "#bbdfc5d9", "#bab700d9", "#60935dd9", "#f8b646d9"];
var PI_2 = 2 * Math.PI;
function useConfetti(c, canvas, colors = COLORS, confettiCount = NUM_CONFETTI) {
  const cofetties = [];
...
var Confeti = class {
  constructor(canvas, c, colors) {
    __publicField(this, "c");
    __publicField(this, "xpos", 0.5);
    __publicField(this, "h");
    __publicField(this, "w");
    __publicField(this, "opacity");
    __publicField(this, "dop");
...
// node_modules/canvas-party/lib/tamplates/trippy.js
function useTrippy(c, canvas) {
  const mapSize = 1024;
  const imgSize = 512;
  ...
  const moveHeightMaps = (t) => {
    dx1 = Math.floor((Math.cos(t * 2e-4 + 0.4 + Math.PI) + 1) / 2 * mapSize / 2);
    dy1 = Math.floor((Math.cos(t * 3e-4 - 0.1) + 1) / 2 * mapSize / 2);
    dx2 = Math.floor((Math.cos(t * -2e-4 + 1.2) + 1) / 2 * mapSize / 2);
    dy2 = Math.floor((Math.cos(t * -3e-4 - 0.8 + Math.PI) + 1) / 2 * mapSize / 2);
  };
  const palettes = [makeRandomPalette(), makeRandomPalette()];
  let palette = [];
  let prevDirection = 1;
  const updatePalette = (t) => {
    const timeScale = 5e-4;
    const x = t * timeScale;
...
// node_modules/canvas-party/lib/tamplates/fireworks.js
var PARTICULES_NUM = 30;
var isActive = false;
var COLORS2 = ["#FF1461", "#18FF92", "#5A87FF", "#FBF38C"];
function useFireworks(c, canvas, colors = COLORS2, count = PARTICULES_NUM) {
  let pointerX = 0;
  let pointerY = 0;
  autoClick();
  ...
  function autoClick() {
    if (isActive)
      return;
    animateParticules(anime_es_default.random(0, canvas.width), anime_es_default.random(0, canvas.height));
    anime_es_default({ duration: 400 }).finished.then(autoClick);
  }
  ...
  function setParticuleDirection(p) {
    var angle = anime_es_default.random(0, 360) * Math.PI / 180;
    var value = anime_es_default.random(50, 180);
    var radius = [-1, 1][anime_es_default.random(0, 1)] * value;
    return {
  ...
  function createParticule(x, y) {
    const p = {
      x,
      y,
      color: colors[anime_es_default.random(0, colors.length - 1)],
      radius: anime_es_default.random(16, 32),
      draw: () => {
        c.beginPath();
...
// node_modules/canvas-party/lib/tamplates/starfield.js
function useStarfield(c, canvas) {
  let w = canvas.width;
  let h = canvas.height;
  ...
  const tick = (time) => {
    ...
    for (var i = 0; i < count; i++) {
      const star = stars[i];
      const x = cx + star.x / (star.z * 1e-3);
      const y = cy + star.y / (star.z * 1e-3);
      if (x < 0 || x >= w || y < 0 || y >= h) {
        continue;
      }
      const d = star.z / 1e3;
...
// node_modules/canvas-party/lib/tamplates/index.js
var tamplates = {
...
// node_modules/canvas-party/lib/index.js
function createCanvasParty(el, options) {
...
//# sourceMappingURL=canvas-party.js.map
